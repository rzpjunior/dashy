...
RUN aws s3 cp s3://$S3_BUCKET_NAME/$BASE_ENV/$CI_COMMIT_BRANCH/example_env.txt ./
RUN mv example_env.txt .env
RUN npm cache clean --force
RUN npm install --force
RUN npm i @vue/cli@5.0.5 --force
RUN npm run build -- --mode development

FROM $CI_REGISTRY/image/nginx/master:latest as deploy
